...
ThinkJS 支持正则路由、规则路由、静态路由等多种路由机制，并且可以基于模块来设置。可以让 URL 解析的同时又不丢失性能。
...
express/koa 是 2 个比较简单的框架，框架本身提供的功能比较简单，项目中需要借助大量的第三方插件才能完成项目的开发。使用很多第三方组件一方便提高了项目的复杂度，另一个方便第三方插件质量参差不齐，也会带来内存泄漏等风险。

koa 使用 ES6 里的 `Generator Function` 解决了异步回调的问题，但 `Generator Function` 只会是个过渡解决方案，会被 ES7 里的 `async/await` 所替代。
